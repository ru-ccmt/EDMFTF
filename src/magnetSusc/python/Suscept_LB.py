...
if __name__ == '__main__':
    ...
    #Qlist=[[0,0,0],[-1,-1,0],[-2,-2,0],[-2,0,0],[-3,-3,0],[-3,-1,0],[-4,-4,0],[-4,-2,0],[-4,0,0],[-5,-5,0],[-5,-3,0],[-5,-1,0],[-6,-6,0],[-6,-4,0],[-6,-2,0],[-6,0,0],
    #       [-7,-7,0],[-7,-5,0],[-7,-3,0],[-7,-1,0],[-8,-8,0],[-8,-6,0],[-8,-4,0],[-8,-2,0],[-8,0,0],[-9,-7,0],[-9,-5,0],[-9,-3,0],[-9,-1,0],[-10,-6,0],[-10,-4,0],
    #       [-10,-2,0],[-10,0,0],[-11,-5,0],[-11,-3,0],[-11,-1,0],[-12,-4,0],[-12,-2,0],[-12,0,0],[-13,-3,0],[-13,-1,0],[-14,-2,0],[-14,0,0],[-15,-1,0],[-16,0,0]]
    ...
    (om,Gf) = ReadGlc(fileglc, nom, cixdm)   # should it be 2*nom instead on nom?
    ...
    for iOm in range(nOm-1,-1,-1):
        ...
        Gamma_Q = zeros((dim2,dim2),dtype=complex)
        for iQ,Q in enumerate(Qlist):
            Qi = k_index(Q)
            ...
            chi0_Q = Cmp_chi0_Q2(gk,nomv,dOm,norb,Qi,k_m_q)  # -G(iom) * G(iOm-iom)
                ...
                for i3 in range(norb):
                    for im in range(2*nomv):
                        for jm in range(2*nomv):
                            Gamma_Q[(i1*norb+i1)*2*nomv+im,(i3*norb+i3)*2*nomv+jm]=GammaS[i1*2*nomv+im,i3*2*nomv+jm]
            if True:
                for i1 in range(norb):
                    for i2 in range(norb):
                        if i1!=i2:
                            for im in range(2*nomv):
                                for jm in range(2*nomv):
                                    Gamma_Q[(i1*norb+i2)*2*nomv+im,(i1*norb+i2)*2*nomv+jm]+=GammaF[i1,i2,im,jm]


            #  chi = (chi0^{-1}+Gamma)^{-1}
            Chi0_Q_inv = linalg.inv(Chi0_Q)
            Chi_S_Q = linalg.inv( Gamma_Q + Chi0_Q_inv )
            ...
            for i1 in range(norb):
                for i2 in range(norb):
                    ...
                    for im in range(2*nomv):
                        for jm in range(2*nomv):
                            csum1 += Chi_S_Q[(i1*norb+i1)*2*nomv+im,(i2*norb+i2)*2*nomv+jm].real
                            csum2 += Chi0_Q[(i1*norb+i1)*2*nomv+im,(i2*norb+i2)*2*nomv+jm].real
                    chiS_sum[i1,i2]=csum1/beta
                    chi0_sum[i1,i2]=csum2/beta
