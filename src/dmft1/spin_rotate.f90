...
SUBROUTINE GetSpinRotation(Rspin,rotij,crotloc,BR1,norbitals,natom,natm,iso,lmax2,iatom,nl,cix,ll,iorbital)
  ...
  REAL*8, intent(in)  :: rotij(3,3,natm), crotloc(3,3,natom), BR1(3,3)
  ...
  REAL*8 :: R3tmp(3,3), tmp(3,3), rotij_cartesian(3,3), BR1inv(3,3), Trans3(3,3), Trans3_proper(3,3)
  ! external
  REAL*8 :: detx
  INTEGER            STDERR
  PARAMETER          (STDERR = 99)
  ...
  DO icase=1,natom
     ...
     do lcase=1,nl(icase)
        ...
        tmp(:,:) = matmul(BR1(:,:),rotij(:,:,latom))
        rotij_cartesian(:,:) = matmul(tmp,BR1inv(:,:))

        Trans3 = matmul(rotij_cartesian,crotloc(:,:,icase))  
        
        Det = detx(Trans3)
        Trans3_proper = transpose( Trans3*Det )
        CALL Angles(phi1,the1,psi1, Trans3_proper )
        CALL Spin_Rotation(Rtmp,phi1,the1,psi1)

        ! Just checking
        CALL Space_Rotation(R3tmp, phi1,the1,psi1 )
        R3tmp = R3tmp-Trans3_proper
        ...
        ! !
        ! !Det = detx(rotij_cartesian)
        ! !rotij_proper = transpose( rotij_cartesian*Det )
