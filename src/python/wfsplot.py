...
def Ind2D(ik1,ik2,N):
    return (2*N-1)*(N-1+ik1)+N-1+ik2
def Ind2D_(ik1,ik2,N):
    return ( N-1+ik1, N-1+ik2 )
...
if __name__ == '__main__':
    ...
    AddLastPoint=False
    orb_plot=range(5)
    orb_plot=[0,0,1,1,0,0,-2,-2,-2,-2,-2,-2,-2,-2,0,0,1,1,0,0,-2,-2,-2,-2,-2,-2,-2,-2,]  # negative entries are not used.
    ...
    if os.path.isfile('UL.dat_') and os.path.getsize('UL.dat_')>0 : COHERENCE=True
    ...
    if (onlyHalfPoints):
        nk0 = int((-1+sqrt(1+8*nkp))/2)
        nk1=2*nk0-1
        print 'nk0=', nk0, 'nk1=', nk1, 'nkp=', nkp
        ...
        #        ika = ik1*nk1+ik2
        #        ikb = ik2*nk1+ik1
        #        print 'ik1=', ik1, 'ik2=', ik2, 'ik=', ik, 'ika=', ika, 'ikb=', ikb
        ...
        Ebnd = zeros((last-first,nk1*nk1),dtype=complex)    
        ik=0
        for ik1 in range(nk0):
            for ik2 in range(ik1,nk0):
                ika = Ind2D( ik1, ik2, nk0)
                ikb = Ind2D( ik1,-ik2, nk0)
                ikc = Ind2D(-ik1, ik2, nk0)
                ikd = Ind2D(-ik1,-ik2, nk0)
                ike = Ind2D( ik2, ik1, nk0)
                ikf = Ind2D( ik2,-ik1, nk0)
                ikg = Ind2D(-ik2, ik1, nk0)
                ikh = Ind2D(-ik2,-ik1, nk0)
                for i in range(len(Ek[ik])):
                    Ebnd[Nmin[ik]+i-first,ika] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikb] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikc] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikd] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ike] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikf] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikg] = Ek[ik][i]
                    Ebnd[Nmin[ik]+i-first,ikh] = Ek[ik][i]
                ik+=1
    else:
        nk1 = int((sqrt(nkp)))
    ...
    if COHERENCE:
        ...
        for i in ifs:
            ...
            for iorb in range(norb):
                if orb_plot[iorb]<0: continue
                if onlyHalfPoints:
                    cohi_ = zeros((nk1,nk1))
                    ik=0
                    for ik1 in range(nk0):
                        for ik2 in range(ik1,nk0):
                            ika = Ind2D_( ik1, ik2, nk0)
                            ikb = Ind2D_( ik1,-ik2, nk0)
                            ikc = Ind2D_(-ik1, ik2, nk0)
                            ikd = Ind2D_(-ik1,-ik2, nk0)
                            ike = Ind2D_( ik2, ik1, nk0)
                            ikf = Ind2D_( ik2,-ik1, nk0)
                            ikg = Ind2D_(-ik2, ik1, nk0)
                            ikh = Ind2D_(-ik2,-ik1, nk0)
                            cohi_[ika[0],ika[1]] = coh[iorb,i,ik]
                            cohi_[ikb[0],ikb[1]] = coh[iorb,i,ik]
                            cohi_[ikc[0],ikc[1]] = coh[iorb,i,ik]
                            cohi_[ikd[0],ikd[1]] = coh[iorb,i,ik]
                            cohi_[ike[0],ike[1]] = coh[iorb,i,ik]
                            cohi_[ikf[0],ikf[1]] = coh[iorb,i,ik]
                            cohi_[ikg[0],ikg[1]] = coh[iorb,i,ik]
                            cohi_[ikh[0],ikh[1]] = coh[iorb,i,ik]
                            ik+=1
