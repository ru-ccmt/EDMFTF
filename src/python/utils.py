...
class DmftEnvironment:
    ...
    def __get_mpi(self):
        ...
        if os.path.isfile(mpifile2):
            self.MPI2 = open(mpifile2, 'r').next().strip()
...
def ParsIndmfi(case):
    ...
    nuimpurities = int(fi.next().split()[0])  # number of independent impurity problems
    iSiginds={}
    for icix in range(nuimpurities):
        dim = int(fi.next().split()[0])  # size of the impurity problem
        imp=[]
        for i in range(dim):
            imp.append(map(int,fi.next().split()[:dim]))
        iSiginds[icix] = imp
    return iSiginds
