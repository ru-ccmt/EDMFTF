...
def check_files(params):
    ...
    for f in files:
        if not os.path.exists(path+'/'+f):
            print (f, 'is not present in the given path', path, '. Please generate this file.\n\nExitting...')
            sys.exit()
...
if __name__=="__main__":
    ...
    # new one
    params = {'path'    : './FeSeee/',    # directory where the phonopy output files are present
    ...
    if  os.path.exists("params.dat"):
        ...
        exec(fo.read())
        fo.close()
        sp = locals()['params']       # stores parameters
        p.update(sp)
    else:
        print ("params.dat was not found so reading default values.\n Be careful...")
