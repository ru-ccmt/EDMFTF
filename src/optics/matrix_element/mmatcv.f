...
!
        ...
        if (l.gt.0)then
          if (l+m-2.ge.0) then 
            xpy1_1=alfa(iv,lm1mm1)*cmplx(iucl1ul(iat,is),0.0d0,8)+    &
     &           beta(iv,lm1mm1)*cmplx(iucl1udl(iat,is),0.0d0,8) +    &
     &           gama(iv,lm1mm1)*cmplx(iucl1ulol(iat,is),0.0d0,8)     

          endif
  
          if (l-m-2.ge.0)then
            xmy1_1=alfa(iv,lm1m1)*cmplx(iucl1ul(iat,is),0.0d0,8)+     &
     &           beta(iv,lm1m1)*cmplx(iucl1udl(iat,is),0.0d0,8)+      &
     &           gama(iv,lm1m1)*cmplx(iucl1ulol(iat,is),0.0d0,8)      
          endif

          if ((l+m-1.ge.0).and.(l-m-1.ge.0))then
             z1_1=alfa(iv,lm1m)*cmplx(iucl1ul(iat,is),0.0d0,8)+       &
     &          beta(iv,lm1m)*cmplx(iucl1udl(iat,is),0.0d0,8)+    &
     &          gama(iv,lm1m)*cmplx(iucl1ulol(iat,is),0.0d0,8) 

          endif
            
        endif

        xpy2_1=alfa(iv,l1mm1)*cmplx(iuclul1(iat,is),0.0d0,8)+         &
     &       beta(iv,l1mm1)*cmplx(iucludl1(iat,is),0.0d0,8) +	    &
     &       gama(iv,l1mm1)*cmplx(iuclulol1(iat,is),0.0d0,8)  

        xmy2_1=alfa(iv,l1m1)*cmplx(iuclul1(iat,is),0.0d0,8)+          &
     &       beta(iv,l1m1)*cmplx(iucludl1(iat,is),0.0d0,8) +	    &
     &       gama(iv,l1m1)*cmplx(iuclulol1(iat,is),0.0d0,8)

        z2_1=alfa(iv,l1m)*cmplx(iuclul1(iat,is),0.0d0,8)+             &
     &     beta(iv,l1m)*cmplx(iucludl1(iat,is),0.0d0,8)+		   &
     &     gama(iv,l1m)*cmplx(iuclulol1(iat,is),0.0d0,8)
...
!       F^6-(l-1,m-1)=f^5_{lm}
        ...
        pxpy_1(iv) = pxpy_1(iv) + CB*(flm4 * xpy1_1 + flm3 * xpy2_1)
        pxmy_1(iv) = pxmy_1(iv) + CB*(flm2 * xmy1_1 + flm1 * xmy2_1)
        pz_1(iv)   = pz_1(iv) + CB*(flm6 * z1_1   + flm5 * z2_1) 
        mxcv_1(iv)=5.0d-1*(pxpy_1(iv)+pxmy_1(iv))
        mycv_1(iv)=5.0d-1*imag*(pxpy_1(iv)-pxmy_1(iv)) !LO TOLTO UN meno davanti...
        mzcv_1(iv)=pz_1(iv)
     enddo
enddo         
...
!-------end check------------------
      ...
      If ((jj1 .Lt. 0) .Or. (jj2 .Lt. 0) .Or. (jj3 .Lt. 0) .Or. (Abs(m1) &
     & .Gt. j1) .Or. (Abs(m2) .Gt. j2) .Or. (Abs(m3) .Gt. j3)) Then
      ...
      If ((jj1 .Gt. 100) .Or. (jj2 .Gt. 100) .Or. (jj3 .Gt. 100)) Then
         Write (6,*)
         Write (6, '("Error(clebgor): angular momenta out of range : ",&
        & 3F5.2)') j1, j2, j3
         Write (6,*)
         Stop
      End If
      If ((mm1+mm2-mm3 .Ne. 0) .Or. (j2+j3 .Lt. j1) .Or. (j1+j3 .Lt. j2) &
     & .Or. (j1+j2 .Lt. j3)) Then
         cleb = 0.d0
         Return
      End If
...
!      integer,intent(in) ::  L1, L2, L3, M1, M2, M3
      ...
      if ((abs(m1).gt.l1).or.(abs(m2).gt.l2).or.(abs(m3).gt.l3)) then
         ...
         do k = kmin, kmax
            tjs = tjs + ((-1)**k) / fact(k) / fact(nint(l1+l2-l3-k))  &
                 / fact(nint(l1-m1-k)) / fact(nint(l2+m2-k)) &
                 / fact(nint(l3-l2+m1+k)) / fact(nint(l3-l1-m2+k))
         enddo

         tjs = tjs * (-1)**(nint(l1-l2-m3))
         tjs = tjs * dsqrt (fact(nint(l1+l2-l3)) * fact(nint(l1-l2+l3))  &
              * fact(nint(-l1+l2+l3)) / fact(nint(l1+l2+l3+1)) &
              * fact(nint(l1+m1)) * fact(nint(l1-m1)) * fact(nint(l2+m2)) * fact(nint(l2-m2)) &
