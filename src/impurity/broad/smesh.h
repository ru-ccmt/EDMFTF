...
public:
  ...
  void MakeTanMesh(int N_, double tanc, double tanw, double b0, double b1);
  void resize(int n);
...
inline void mesh1D::MakeTanMesh(int N_, double tanc, double tanw, double b0, double b1){
...
***************************************************************************/
  const int M2=M/2; M=2*M2;
  const int Nd = (iremove>=0) ? ome.size()+M-1 : ome.size()+M;
  ...
  if (m.begin()==ome.begin()){
    ...
    if (iremove>=0){
      ...
      std::copy(omadd.begin(), omadd.begin()+M, m.begin()+omt.size()-1);
    } else{
      std::copy(omt.begin(), omt.end(), m.begin());
      std::copy(omadd.begin(), omadd.begin()+M, m.begin()+omt.size());
    }
  } else {
    m.resize(Nd);
    if (iremove>=0){
      ...
      std::copy(omadd.begin(), omadd.begin()+M, m.begin()+ome.size()-1);
    } else{
      std::copy(ome.begin(), ome.end(), m.begin());
      std::copy(omadd.begin(), omadd.begin()+M, m.begin()+ome.size());
    }
  }
