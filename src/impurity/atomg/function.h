...
// arrays of functionb<T> rather than functions1D<T>.				  //
...
//******************************************************************//
// One dimensional functions derived from functionb<T>. It has it's  //
...
public:
  ...
  void Product(const function2D<T>& A, const functionb<T>& x, double alpha=1., double beta=0.);
};
...
//**********************************************************************//
// Two dimentional function<T> derived from functionb<T>. It consists	//
// of an array of functionb<T> rather tham functionb1D<T>.		//
...
public:
  ...
  function2D(const function2D<T>& m);
  ~function2D();
  ...
  void TensorProduct(const functionb<T>& x, const functionb<T>& y, double alpha=1);
  void Product(const function2D& A, const function2D& B, const T& alpha=1, const T& beta=0);
...
{
  SUNCA_LOG(if (N!=m.size()) cerr << "Functions not of equal length! Can't sum!" << std::endl;)
...
{
  SUNCA_LOG(if (N<=0) cerr << "Size of functionb is non positive! "<<N<<std::endl;)
...
{
  SUNCA_LOG(if (N<m.N) cerr<<"Size of function too small in SetProduct!"<<std::endl;);
...
template<class T>
function2D<T>::function2D(const function2D<T>& m) :  N0(m.N0), Nd0(m.Nd0), N(m.N), Nd(m.N)
{
  memory = operator new (sizeof(funProxy<T>)*N0+sizeof(T)*Nd0*N0+HPoffset);
  Assert(memory!=NULL,"Out of memory");
  f = new (memory) funProxy<T>[N0];
...
template <class T>
inline void function2D<T>::Product(const function2D& A, const function2D& B, const T& alpha, const T& beta)
{
...
template <class T>
inline void function2D<T>::TensorProduct(const functionb<T>& x, const functionb<T>& y, double alpha)
{
  if (Nd0<y.size()||N0<x.size()) {std::cerr<<"Matrix sizes not correct"<<std::endl;}
  resize(x.size(),y.size());
  xger(y.size(),x.size(),alpha,y.MemPt(),1,x.MemPt(),1,MemPt(),Nd0);
}

template <class T>
inline void function1D<T>::Product(const function2D<T>& A, const functionb<T>& x, double alpha, double beta)
{
  if (A.size_Nd()!=x.size()) {std::cerr<<"Matrix sizes not correct"<<std::endl;}
  resize(A.size_N());
  xgemv<_Normal>(A.size_Nd(), A.size_N(), alpha, A.MemPt(), A.fullsize_Nd(), x.MemPt(), beta, this->MemPt());
}
...
template <class T>
inline T logpart(double a, double b, const T& x)
{
  return log((b-x)/(a-x));
}
template <class T, class mesh>
T KramarsKronig(const functionb<double>& fi, const mesh& om, const T& x, int i0, double S0)
{
  T sum=0;
