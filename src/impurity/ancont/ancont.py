...
def chi2(gweigh, vary, gwfix, fixed, sqmc, ifunr, ifuni, iom, intg, om, rfun, expand, ders, alphas):
    ...
    if (globalc%global_print == 0):
        ...
        for im in range(len(iom)):
            ...
            print >> fn, iom[im], gc.real, gc.imag, sqmc[im,0], sqmc[im,1]
...
#    print 'chi2=', chi, 'chi3=', chi3, 'chi4=', chi4
    ...
    if ((globalc-1)%global_print == 0):
        ...
        for im in range(len(om)):
            ...
            print >> fh, om[im], csum, zsum.real+sinfty, zsum.imag
        ...
        print >> fm, 'nom=', params['nom'][0],
        print >> fm, 'beta=', params['beta'][0],
        print >> fm, 'b=', params['b'][0],
        print >> fm, 'Ng=', params['Ng'][0],
        print >> fm, 'wexp=', params['wexp'][0],
        print >> fm, 'ifit=', params['ifit'][0],
        print >> fm, 'alpha3=', params['alpha3'][0],
        print >> fm, 'alpha4=', params['alpha4'][0]
        for i in range(len(gweigh)):
            print >> fm, i, gpos[i], gweigh[i]
...
if __name__ == '__main__':
    ...
    for var in params.keys():                    # We go over all parameters and evaluate them
...
#    (Frest_real, ders_rest) = hge.RealParts(om, cutoff)
    ...
    for im in range(len(om)):
        ...
        for i in range(len(fixed)):
            zsum += rfunc[fixed[i]-1,im]*gwfix[i]
        print >> f3, om[im], zsum.real+sinfty, zsum.imag
