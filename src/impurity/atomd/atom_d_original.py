...
if __name__ == '__main__':
    ...
    if (len(glob.glob(ftrans))!=0):
        ...
        if len(Sigind)==(2*l+1):
            ...
            Sigind=SigindN
        #if len(CF)==(2*l+1):
        #    CFN = zeros((2*(2*l+1), 2*(2*l+1)), dtype=complex)
        #    CFN[:dn,:dn] = CF
        #    CFN[dn:,dn:] = CF
        #    CF = CFN
        ...
        T2C = TS2C(l)
        Sigind = identity(2*(2*l+1), dtype=int)
    ...
    #global_flip = [Sigind[b,b]-1 for b in range(len(Sigind)/2)]
    ...
    op = operateLS(2*l+1) # operators class
    ...
    wstates = baseN(2*l+1,prop)

    SO = SpinOrbitM(l,T2C) # Spin orbit matrix
    
    UC = CoulUsC1(l,T2C)     # Coulomb repulsion matrix
    ...
    #print "U Hartree="         
    #for m1 in range(2*l+1):
    #    print ("%10.6f"*(2*l+1)) % tuple(UHa[m1,:])
    #print "U Fock="         
    #for m1 in range(2*l+1):
    #    print ("%10.6f"*(2*l+1)) % tuple(UFo[m1,:])
    ...
    #        #print "(m2,m3)=(", m2, m3, ")"
    #        for m1 in range(2*l+1):
    #            print "%10.6f" % (UHa[m1,m2,m3]-UFo[m1,m2,m3]),
    #        print
    #    
    #print 'Opposite spins='
    #for m2 in range(2*l+1):
    #    for m3 in range(2*l+1):
    #        #print "(m2,m3)=(", m2, m3, ")"
    #        for m1 in range(2*l+1):
    #            print "%10.6f" % UHa[m1,m2,m3],
    #        print
    #        
    print 'Together='
    for bs1 in baths:
    ...
    for ni,ns in enumerate(wstates):
        ...
        print >> fh_info, 'E=', '%f '*len(eig[0]) % tuple(eig[0])
    ...
    print >> fcix, 1, len(lowE), len(baths), low_maxsize
    print >> fcix, '# baths, dimension, symmetry'
    
    for ib in range(len(baths)):
        print >> fcix, ib, '  ', 1, Sigind[ib,ib]-1, '  ', global_flip[ib] #Sigind[ib,ib]-1
    ...
    for i in range(len(lowE)):
        ...
        ip = iwp[1]
        wstate = wstates[iw]
        print >> fcix, "%3d  %2d %2d %4.1f %2d " % (i+1, sum(wstate[0]), 0, sum(wstate[1])/2., len(lowE[i][1])),#len(Enes[ii])),
        for ib in range(len(baths)):
            ifinal = iFinal[ii,ib]
    ...
    print >> fcix, 1, len(pseudostates), len(baths), maxs
