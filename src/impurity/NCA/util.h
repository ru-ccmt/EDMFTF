...
std::string NameOfFile__(const std::string& name, int n1, int n2, int n3, int m1=2, int m2=2, int m3=2){
  std::stringstream str;
  str<<name<<".";
  for (int i=1; i<m1; i++){
    if (n1<power(10,i)) str<<"0";
...
inline T findRoot(const T& x0, functor& f, double tol, int MaxSteps)
{
  ...
  std::cerr << "Can't find root after " << MaxSteps;
  std::cerr << " iterations. The value of the function is " << fa << std::endl;
