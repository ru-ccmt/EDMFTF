...
void Print_NCA(ostream& stream, int baths, const vector<vector<int> >& base, const vector<int>& Ns, const vector<int>& Mtot,
...
void Print_OCA(ostream& stream, int baths, const vector<vector<int> >& base, 
...
int main()
{
  ...
  string str;
  cin>>str;
  list<int> core_and_valence;
  parsInputChoice(str, core_and_valence, base.size());
  
  cout<<" Which of those states would you like to treat dynamically (fully dress) ?"<<endl;
  cout<<"  Enter your choice in format [xx-yy]+[,xx]+ (Example: 0-12,15,20)"<<endl;
  cin>>str;
  list<int> valence;
  parsInputChoice(str, valence, base.size());
  
  list<int> core;
  for (list<int>::iterator i=core_and_valence.begin(); i!=core_and_valence.end(); i++){
    list<int>::iterator l = find(valence.begin(), valence.end(), *i);
    if (l==valence.end()) core.push_back(*i);
  }

  vector<vector<int> > cv_base(valence.size()+core.size());
  map<vector<int>,int> cv_index;
  vector<int> cv_Mtot(cv_base.size());
  vector<int> cv_degeneracy(cv_base.size());
  vector<vector<int> > cv_ncab(cv_base.size());
  vector<vector<int> > cv_ncaf(cv_base.size());
  
  int l=0;
  for (list<int>::iterator i=valence.begin(); i!=valence.end(); i++,l++) cv_base[l] = base[*i];
  for (list<int>::iterator i=core.begin(); i!=core.end(); i++,l++) cv_base[l] = base[*i];
  SetUp(cv_base, Ns, cv_index, cv_Mtot, cv_degeneracy);
  CalcNCADiag(baths, cv_base, Ns, cv_index, cv_degeneracy, cv_ncab, cv_ncaf);

  vector<vector<int> > valence_base(valence.size());
  map<vector<int>,int> valence_index;
  vector<int> valence_Mtot(valence_base.size());
  vector<int> valence_degeneracy(valence_base.size());

  vector<vector<vector<int> > > sobb(valence_base.size());
  vector<vector<vector<int> > > sobf(valence_base.size());
  vector<vector<vector<int> > > sofb(valence_base.size());
  vector<vector<vector<int> > > soff(valence_base.size());
  vector<vector<vector<int> > > sogl(valence_base.size());
  
  vector<vector<int> > prbb(valence_base.size());
  vector<vector<int> > prbf(valence_base.size());
  vector<vector<int> > prfb(valence_base.size());
  vector<vector<int> > prff(valence_base.size());
  vector<vector<int> > prgl(valence_base.size());
  
  l=0;
  for (list<int>::iterator i=valence.begin(); i!=valence.end(); i++,l++) valence_base[l] = base[*i];
  
  SetUp(valence_base, Ns, valence_index, valence_Mtot, valence_degeneracy);
  CalcUNCADiag_(baths, valence_base, Ns, valence_index, valence_degeneracy, sobb, sobf, sofb, soff, sogl, prbb, prbf, prfb, prff, prgl);
  ...
  out<<valence.size()<<" "<<core.size()<<"  # Number of baths it's degeneracy an number of local valence and local core states"<<endl;
  Print_NCA(out, baths, cv_base, Ns, cv_Mtot, cv_degeneracy, cv_ncab,  cv_ncaf);
  Print_OCA(out, baths, valence_base, sobb, sobf, sofb, soff, sogl);
  Print(out, baths, valence_base.size(), sobb, sobf, sofb, soff, sogl, prbb, prbf, prfb, prff, prgl);
