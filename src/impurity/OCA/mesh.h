...
public:
  ...
  friend void MeshFor2Peaks_SmallMesh<mesh1D>(mesh1D& m, int MaxPoints, const mesh1D& ome, const parMeshFor2Peaks& par);
};
...
inline void mesh::InterpBoth(const double x, int& ix, intpar& ip) const
{
  ...
  ip.i = ix = i;
  ip.p = (x-om[i])*delta[i];
  SUNCA_LOG(if ((ip.p>1.0 || ip.p<0) && i!=0 && i!=(N-2)) std::cerr<<"Variable p="<<ip.p<<" at i="<<i<<" flag1="<<debug_flag1<<" flag2="<<debug_flag2<<std::endl);
}
...
{ std::cerr << "Bug!! InterpLeft<> should not be called"<<std::endl; return 0;}
template <int left>
