...
inline void NState::apply(const functionb<function2D<double> >& FM, const functionb<int>& Fi, const NState& C)
{
  //t_apply.start();
...
inline Number NState::Project_to_Pra(const NState& s, functionb<Number>& Proj){
  if (s.istate!=istate) {
