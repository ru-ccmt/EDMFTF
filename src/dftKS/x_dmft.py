...
def PrepareDefinitionFile(idmf, mode, case, cixs, updn, dnup, so, para, scratch, cmplx, _band='', m_ext=''):
    ...
    if mode=='g':
        print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_CDOS,"'"+case+".cdos"+m_ext+"'",  "'unknown'","'formatted'",0) # total DOS written here

    print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (59, "'"+case+".energy"+so+dnup+para+"'", "'unknown'","'formatted'",0) #
    print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (60, "'"+case+".energy"+so+updn+para+"'", "'old'","'formatted'",0) # 

    # Appending to the def file for correlated atoms
    # for each correlated (atom,l)-block one file
    for c in cixs:
        print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_S+c, "'"+'sig.inp'+str(c)+m_ext+_band+"'",        "'old'",    "'formatted'",0) # self-energy for correlated

        if mode=='g':
            print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_G+c,"'"+case+'.gc'+str(c)+m_ext+"'",  "'unknown'","'formatted'",0) # only correlated green's functions
            print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_D+c,"'"+case+'.dlt'+str(c)+m_ext+"'", "'unknown'","'formatted'",0) # delta's for correlated
            print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_IMP+c,"'"+case+'.Eimp'+str(c)+m_ext+"'", "'unknown'","'formatted'",0) # Impurity levels for correlated
            
    if mode in ['e', 'p']:
        print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_IMP,"'eigvals"+m_ext+".dat'", "'unknown'","'formatted'",0) # eigenvalues will be stored

    if mode=='u':
        print >> fdef, "%3d, %-15s, %-10s, %-13s, %-4d" % (IND_UDMF,"'Udmft"+m_ext+updn+".0'", "'unknown'","'unformatted'",0) # eigenvalues will be stored
...
if __name__=='__main__':
	...
	if (options._no_group):
        ...
        PrepareInFile(idmf, mode, w2k.case, updn,1)
        PrepareDefinitionFile(idmf, mode, w2k.case, cixs, updn, dnup, so, para, w2k.SCRATCH, cmplx, '_band')

        nom = inl.om_npts 
        aom = inl.om_emin  
        bom = inl.om_emax  

        cmd = dmfe.ROOT+'/ssplit.py ' #-n '+str(nom)+' -t '+str(ntail)
        ...
        if not options.def_only:
            runExternal('dmft'+idmf, dmfe.ROOT, dmfe.MPI2, options.nom, options.ntail, 'dmft','',False)
    else:
