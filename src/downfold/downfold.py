...
def Print(fh, R, tHr, DVinv):
    ...
    for l1 in range(shape(tHr)[0]):
        ...
        for l2 in range(shape(tHr)[1]):
            #if abs(tHr[l1,l2])<1e-10:
            #    tHr[l1,l1]=0.0
            #if abs(tHr[l1,l2].imag)<1e-4:
            #print >> fh, "%8.4f," % (tHr[l1,l2].real,),
            #else:
            #print >> fh, "%8.4f," % (tHr[l1,l2].real,),
...
if __name__ == '__main__':
    ...
    for ikp in range(nkp):
        ...
        for isym in range(nsymop):
            ...
            # Energies in the choosen interval
    ...
    print >> fh
    print >> fh, "lattice_unit=[", tuple(DV[0]), ',', tuple(DV[1]), ',', tuple(DV[2]), "]"
