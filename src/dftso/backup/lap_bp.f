...
!              Update A(1:i,i)
!
               CALL DGEMV( 'No transpose', I, N-I, -ONE, ABP(I1I1), &
                           LDI, W( I, IW+1 ), LDW, ONE, ABP(I1I), 1 )
               CALL DGEMV( 'No transpose', I, N-I, -ONE, W( 1, IW+1 ), &
                           LDW, ABP(III1), LDI, ONE, ABP(I1I), 1 )
            END IF
            IF( I.GT.1 ) THEN
!
...
!
    ...
    4          CONTINUE
		 ...
		 IF( J.NE.1) THEN 
		   CALL DGEMV( 'NoTranspose', J-1, JB, ONE,  &
      		               ABP(I1J), LDJ, ABP(IJI), 1,  &
                               ONE, W(1, IW), 1) 
		   CALL DGEMV( 'Transpose', J-1, JB, ONE,  &
      		               ABP(I1J), LDJ, ABP(I1I), 1,  &
                               ONE, W(J, IW), 1)
		 END IF 
!	         write(*,*) 'HB=',HB,' J=',J,' JB=',JB,' I=',I
		 CALL DSYMV( 'U', JB, ONE, ABP(IJJ), LDJ,  &
      			     ABP(IJI), 1, ONE, W( J, IW), 1)
    5	       CONTINUE
               IF( I.LT.N ) THEN
                  CALL DGEMV( 'Transpose', I-1, N-I, ONE, W( 1, IW+1 ), &
                              LDW, ABP(I1I), 1, ZERO, W( I+1, IW ), 1 )
                  CALL DGEMV( 'No transpose', I-1, N-I, -ONE, &
                              ABP(I1I1), LDI, W( I+1, IW ), 1, ONE, &
                              W( 1, IW ), 1 )
                  CALL DGEMV( 'Transpose', I-1, N-I, ONE, ABP(I1I1), &
                              LDI, ABP(I1I), 1, ZERO, W( I+1, IW ), 1 )
                  CALL DGEMV( 'No transpose', I-1, N-I, -ONE, &
	    ...
	    if ( info.ne.0 ) goto 20
            IF( K+KB.LE.N ) THEN
               ...
               DO 7 KP = 0, KPEND
                  ...
                  CALL DTRSM( 'Left', 'U', 'Transpose', 'Non-unit', &
                              KB, KKB, ONE, ABP(KTMP1), K+KB-1, &
                              ABP( KTMP2 ), KK+KKB-1 )
    7          CONTINUE
...
!$ DOACROSS Local (KP,KK,KKB,KKK,KKKB,KTMP1,KTMP2,KTMP3)
               DO 5 KP = 0, KPEND
                  ...
                  CALL DSYRK( 'U', 'Transpose', KKB, &
                        KB, -ONE, ABP( KTMP1 ), KK+KKB-1, ONE, &
                        ABP( KTMP2 ), KK+KKB-1  )
                  IF( KK+KKB.LE.N ) THEN
                     DO 3 KKK = KK+KKB, N, HB
                        KKKB = MIN( HB, N-KKK+1 )
                        KTMP1 = IJ2K( 'U', K, KKK, N, HB )
                        KTMP2 = IJ2K( 'U', K, KK, N, HB )
                        KTMP3 = IJ2K( 'U', KK, KKK, N, HB )
                        CALL DGEMM( 'Transpose', 'No transpose', &
                           KKB, KKKB, KB, -ONE, ABP( KTMP2 ),  &
                           KK+KKB-1, &
                           ABP ( KTMP1 ), KKK+KKKB-1, ONE, ABP( KTMP3 ), &
                           KKK+KKKB-1 )
    3                CONTINUE
                  END IF
    5          CONTINUE
...
!
      END
      SUBROUTINE DSHEVX( JOBZ, RANGE, UPLO, N, AP, VL, VU, IL, IU, &
                         ABSTOL, M, W, Z, LDZ, WORK, IWORK, IFAIL, &
                         HBTRD, NUME, NBVL, INFO )
!
!
!  Block packed algorithms                                           1997
!  Based on LAPACK routines                              Dieter Kvasnicka
...
!     ..
!     .. External Subroutines ..
      EXTERNAL           DCOPY, DOPGTR, DOPMTR, DSCAL, DSHTRD, DSTEZ2, &
...
!      tarray(1)=dtime(tarray)
      CALL DSHTRD( UPLO, N, AP, WORK( INDD ), WORK( INDE ), &
                   WORK( INDTAU ), WORK( INDWRK ), LLWORK, HBTRD,  &
                   IINFO )
...
!
	...
	M=NUME
      END IF
      CALL DSTEZ2( RANGE, ORDER, N, VLL, VUU, IL, IU, ABSTLL, &
                   WORK( INDD ), WORK( INDE ), M, NSPLIT, W, &
                   IWORK( INDIBL ), IWORK( INDISP ), WORK( INDWRK ), &
                   IWORK( INDIWO ), NUME, NBVL, INFO )
...
!		  write (*,*) 'dsygst: ikk,k,kb=',ikk,k,kb 
		  ...
		  IF( INFO.NE.0) GO TO 20
			...
			IK2K2 = IJ2K( UPLO, KK, KK, N, HB )
                        CALL DTRSM( 'Left', UPLO, 'Trans', 'Non-unit', &
                                    KB, KKB, ONE, BBP( IKK ), K+KB-1, &
                                    ABP( IKK2 ), KK+KKB-1 )
                        CALL DSYMM( 'Left', UPLO, KB, KKB, -HALF, &
                                    ABP( IKK ), K+KB-1, BBP( IKK2 ),  &
                                    KK+KKB-1, &
                                    ONE, ABP( IKK2 ), KK+KKB-1 )
    9                CONTINUE
!
!                       dsyr2k block-wise
!
			...
			IK2K2 = IJ2K( UPLO, KK, KK, N, HB )
                        IF (  KK .GT. K+KB ) THEN
                           DO 2 KKK = K+KB, KK-KKB, HB
                              KKKB = MIN( N-KKK+1, HB )
			      IKK3 = IJ2K( UPLO, K, KKK, N, HB )
			      IK3K2 = IJ2K( UPLO, KKK, KK, N, HB )
                              CALL DGEMM( 'Transpose', 'No transpose', &
                                          HB, KKB, HB, &
                                          -ONE, ABP( IKK3 ), KKK+KKKB-1, &
                                          BBP( IKK2 ), KK+KKB-1, &
                                          ONE, ABP( IK3K2 ), KK+KKB-1)
                              CALL DGEMM( 'Transpose', 'No transpose', &
                                          HB, KKB, HB, &
                                          -ONE, BBP( IKK3 ), KKK+KKKB-1, &
                                          ABP( IKK2 ), KK+KKB-1, &
                                          ONE, ABP( IK3K2 ), KK+KKB-1)
   2                      CONTINUE
                        ENDIF
                        CALL DSYR2K( UPLO, 'Transpose', KKB, KB, -ONE, &
                                  ABP( IKK2 ), KK+KKB-1, BBP( IKK2 ),  &
                                  KK+KKB-1, &
                                  ONE, ABP( IK2K2 ), KK+KKB-1 )
   8                 CONTINUE
...
!
                        IF (  KK .GT. K+KB ) THEN
                           DO 3 KKK = K+KB, KK-KKB, HB
                              KKKB = MIN( N-KKK+1, HB )
			      IKK3 = IJ2K( UPLO, K, KKK, N, HB )
			      IK3K2 = IJ2K( UPLO, KKK, KK, N, HB )
                              CALL DGEMM( 'No trans', 'No trans', &
                                          HB, KKB, HB, &
                                          -ONE, ABP( IKK3 ), KKK+KKKB-1, &
                                          BBP( IK3K2 ), KK+KKB-1, &
                                          ONE, ABP( IKK2 ), KK+KKB-1)
...
!
      END
      SUBROUTINE DSHTR4( UPLO, N, ABP, D, E, TAU, WORK, LWORK, HB,  &
                         INFO )
!
!
!  Block packed algorithms                                           1997
...
!           the matrix
!
            CALL DLATD4( UPLO, LDI, IB, ABP, E, TAU, WORK, &
                         LDWORK, NB )
...
!              write(*,*) 'DSYR2K I=',I,' J=',J
	      ...
	      IF( J.NE.1 ) THEN
		CALL DGEMM( 'NoTranspose', 'Transpose', J-1, NB, IB, &
      		       -ONE, ABP(I1I), LDI, WORK(J), LDWORK, &
      			ONE, ABP(I1J), LDJ)
                CALL DGEMM( 'NoTranspose', 'Transpose', J-1, NB, IB, &
      		       -ONE, WORK, LDWORK, ABP(IJI), LDI, &
      			ONE, ABP(I1J), LDJ)
	      END IF
	      CALL DSYR2K( UPLO, 'NoTranspose', NB, IB, &
      		      -ONE, ABP(IJI), LDI, WORK(J), LDWORK, &
      		       ONE, ABP(IJJ), LDJ)
    5       CONTINUE
...
!
      END
      SUBROUTINE DSHTRD( UPLO, N, ABP, D, E,  &
                         TAU, WORK, LWORK, HB,  &
                         INFO )
!
!
...
!
        call DSHTR4( UPLO, N, ABP, D, E, TAU, WORK, LWORK, NB,  &
                         INFO )
!      call CPUTIM(CP(3))
!
!      convert back to upper triangular packed storage format
...
!
      END
      SUBROUTINE DSTEZ2( RANGE, ORDER, N, VL, VU, IL, IU, ABSTOL, D, E, &
...
!     .. Local Scalars ..
      ...
      INTEGER            IB, IBEGIN, IDISCL, IDISCU, IE, IEND, IINFO, &
                         ...
                         ITMP1, IW, IWOFF, J, JB, JDISC, JE, NB, &
                         NWU
...
!
      ...
      ELSE IF( N.LT.0 ) THEN
         INFO = -3
      ELSE IF( ( ( IRANGE.EQ.2) .OR. (IRANGE.EQ.4) ) .AND.  &
               VL.GE.VU ) THEN
         INFO = -5
...
!
      IF( N.EQ.1 ) THEN
         NSPLIT = 1
         ISPLIT( 1 ) = 1
         IF( ( ( IRANGE.EQ.2 ) .OR. ( IRANGE.EQ.4 ) ) .AND.  &
             ( VL.GE.D( 1 ) .OR. VU.LT.D( 1 ) ) ) THEN
            M = 0
         ELSE
            W( 1 ) = D( 1 )
            IBLOCK( 1 ) = 1
...
!
      IF( ( IORDER.EQ.1 .AND. NSPLIT.GT.1 ) .OR. &
          ( IORDER.EQ.4 .AND. M.GT.NUME ) ) THEN
         DO 150 JE = 1, M - 1
            IE = 0
            TMP1 = W( JE )
            DO 140 J = JE + 1, M
...
!
      ITMP = MOD(J-1,HB)
      IF ( UPPER ) THEN
         IJ2K = I + HB*HB*((J-1)/HB)*((J-1)/HB+1)/2 +  &
                ITMP*MIN(((J-1)/HB+1)*HB,N)
      ELSE
         IJ2K = I+(2*N-J)*(J-1)/2 + (HB*(HB-1)/2) * ((J-1)/HB) + &
                ITMP*(ITMP+1)/2
      END IF
      RETURN
!
...
!
      END
      SUBROUTINE ZHHEVX( JOBZ, RANGE, UPLO, N, AP, VL, VU, IL, IU, &
                         ABSTOL, M, W, Z, LDZ, WORK, RWORK, IWORK, &
                         IFAIL, HBTRD, NUME, NBVL, INFO )
!
!
!  Block packed algorithms                                           1997
!  Based on LAPACK routines                              Dieter Kvasnicka
...
!     ..
!     .. External Subroutines ..
      EXTERNAL           DCOPY, DSCAL, DSTEZ2, DSTERF, XERBLA, ZDSCAL, &
                         ZHHTRD, ZSTEIN, ZSTEQR, ZSWAP, ZUPGTR, ZUPMTR
!     ..
!     .. Intrinsic Functions ..
...
!
      ...
      INDWRK = INDTAU + N
      LLWORK = N*HBTRD
      CALL ZHHTRD( UPLO, N, AP, RWORK( INDD ), RWORK( INDE ), &
                   WORK( INDTAU ), WORK( INDWRK ), LLWORK, HBTRD,  &
                   IINFO )
!
!     If all eigenvalues are desired and ABSTOL is less than or equal
...
!		  write (*,*) 'zhegst: ikk,k,kb=',ikk,k,kb 
                  CALL ZHEGST( ITYPE, UPLO, KB, ABP( IKK ), K+KB-1, &
                               BBP( IKK ), K+KB-1, INFO )
		  IF( INFO.NE.0) GO TO 10
			...
			IKK2 = IJ2K( UPLO, K, KK, N, HB )
			IK2K2 = IJ2K( UPLO, KK, KK, N, HB )
                        CALL ZTRSM( 'Left', UPLO, 'Conj', 'Non-unit', &
                                    KB, KKB, ONE, BBP( IKK ), K+KB-1, &
                                    ABP( IKK2 ), KK+KKB-1 )
                        CALL ZHEMM( 'Left', UPLO, KB, KKB, -HALF, &
                                    ABP( IKK ), K+KB-1, BBP( IKK2 ),  &
                                    KK+KKB-1, &
                                    ONE, ABP( IKK2 ), KK+KKB-1 )
    9                CONTINUE
...
!
			...
			IK2K2 = IJ2K( UPLO, KK, KK, N, HB )
			      ...
			      IKK3 = IJ2K( UPLO, K, KKK, N, HB )
			      IK3K2 = IJ2K( UPLO, KKK, KK, N, HB )
                              CALL ZGEMM( 'Conjugate', 'No transpose', &
                                          HB, KKB, HB, &
                                          -ONE, ABP( IKK3 ), KKK+KKKB-1, &
                                          BBP( IKK2 ), KK+KKB-1, &
                                          ONE, ABP( IK3K2 ), KK+KKB-1)
   ...
   2                      CONTINUE
                        ENDIF
                        CALL ZHER2K( UPLO, 'Conjugate', KKB, KB, -ONE, &
                                  ABP( IKK2 ), KK+KKB-1, BBP( IKK2 ),  &
                                  KK+KKB-1, &
                                  ONE, ABP( IK2K2 ), KK+KKB-1 )
   8                 CONTINUE
...
!
      END
      SUBROUTINE ZHHTR4( UPLO, N, ABP, D, E, TAU, WORK, LWORK, HB,  &
                         INFO )
!
!
!
...
!           the matrix
!
            CALL ZLATD4( UPLO, LDI, IB, ABP, E, TAU, WORK, &
                         LDWORK, NB )
!	    T1 = T1 + dtime2(lasttime)
!
!           Update the unreduced submatrix A(1:i-1,1:i-1), using an
...
!              write(*,*) 'ZHER2K I=',I,' J=',J
	      ...
	      IJJ = IJ2K(UPLO, J, J, N, NB)
	      IF( J.NE.1 ) THEN
		CALL ZGEMM( 'NoTranspose', 'Conjugate', J-1, NB, IB, &
      		       -ONE, ABP(I1I), LDI, WORK(J), LDWORK, &
      			ONE, ABP(I1J), LDJ)
                CALL ZGEMM( 'NoTranspose', 'Conjugate', J-1, NB, IB, &
      		       -ONE, WORK, LDWORK, ABP(IJI), LDI, &
      			ONE, ABP(I1J), LDJ)
	      END IF
	      CALL ZHER2K( UPLO, 'NoTranspose', NB, IB, &
      		      -ONE, ABP(IJI), LDI, WORK(J), LDWORK, &
      		       ONE, ABP(IJJ), LDJ)
    5       CONTINUE
!	    T2 = T2 + dtime2(lasttime)
...
!
      END
      SUBROUTINE ZHHTRD( UPLO, N, ABP, D, E,  &
                         TAU, WORK, LWORK, HB,  &
                         INFO )
!
!
...
!      compute the tridiagonalization of A
!
        call ZHHTR4( UPLO, N, ABP, D, E, TAU, WORK, LWORK, NB,  &
                         INFO )
!      call CPUTIM(CP(3))
!
!      convert back to upper triangular packed storage format
...
!     .. Local Scalars ..
      INTEGER            I, IW, LDI, LDJ
      COMPLEX*16         ALPHA
      INTEGER            I1I, I1I1, I1J, III,  &
                         IJI, IJJ, II1I, III1, J, JB 
!     ..
!     .. External Subroutines ..
...
!
	       ...
	       CALL ZLACGV( N-I, W( I, IW+1 ), LDW )
               CALL ZGEMV( 'No transpose', I, N-I, -ONE, ABP(I1I1), &
                           LDI, W( I, IW+1 ), LDW, ONE, ABP(I1I), 1 )
	       CALL ZLACGV( N-I, W( I, IW+1 ), LDW )
...
!
    ...
    4          CONTINUE
		 ...
		 IJJ = IJ2K( UPLO, J, J, N, HB ) 
		 IF( J.NE.1) THEN 
		   CALL ZGEMV( 'NoTranspose', J-1, JB, ONE,  &
      		               ABP(I1J), LDJ, ABP(IJI), 1,  &
                               ONE, W(1, IW), 1) 
		   CALL ZGEMV( 'Conjugate Transpose', J-1, JB, ONE,  &
      		               ABP(I1J), LDJ, ABP(I1I), 1,  &
                               ONE, W(J, IW), 1)
		 END IF 
!	         write(*,*) 'HB=',HB,' J=',J,' JB=',JB,' I=',I
		 CALL ZHEMV( 'U', JB, ONE, ABP(IJJ), LDJ,  &
      			     ABP(IJI), 1, ONE, W( J, IW), 1)
    5	       CONTINUE
               IF( I.LT.N ) THEN
                  CALL ZGEMV( 'Conjugate Transpose', I-1, N-I,  &
                              ONE, W( 1, IW+1 ), &
                              LDW, ABP(I1I), 1, ZERO, W( I+1, IW ), 1 )
         ...
         DO 10 K = 1, N, HB
            ...
            IF( K+KB.LE.N ) THEN
               ...
               DO 7 KP = 0, KPEND
                  ...
                  KKB = MIN( HB, N-KK+1 )
                  KTMP2 = IJ2K( 'U', K, KK, N, HB )
                  CALL ZTRSM( 'Left', 'U', 'ConjugateTranspose',  &
                              'Non-unit', &
                              KB, KKB, ONE, ABP(KTMP1), K+KB-1, &
                              ABP( KTMP2 ), KK+KKB-1 )
    7          CONTINUE
...
!$ DOACROSS Local (KP,KK,KKB,KKK,KKKB,KTMP1,KTMP2,KTMP3)
               DO 5 KP = 0, KPEND
                  ...
                  KTMP1 = IJ2K( 'U', K, KK, N, HB )
                  KTMP2 = IJ2K( 'U', KK, KK, N, HB )
                  CALL ZHERK( 'U', 'ConjugateTranspose', KKB, &
                        KB, -ONE, ABP( KTMP1 ), KK+KKB-1, ONE, &
                        ABP( KTMP2 ), KK+KKB-1  )
                  IF( KK+KKB.LE.N ) THEN
                     DO 3 KKK = KK+KKB, N, HB
