      subroutine init(deffn,errfn,e,vru,emm,fl,jspin,kpot,ipr &
                     ,th,ph,irlotot)
...
! write also on dummy vector file 
          if(isi.eq.1)then
            ...
            write(53,'(100(f9.5))')((elo(l,nn,ity,isi),l=0,lomax), &
                      nn=1,nloat)  
          endif
!rschmid
...
!              loor(l,ity) = .true.
              ilo(l,ity) = k
         IF((lapw(l,ity).AND.ilo(l,ity).EQ.1).OR.ilo(l,ity).eq.2) &
                 loor(l,ity)=.true.
...
! end of orbital potential input
      ...
      write(6,*)
      write(6,*) &
      ' **** spin-orbit is excluded from the calculation ****'
      write(6,*)
      write(8,*)
      write(8,*) &
      ' **** spin-orbit is excluded from the calculation ****'
