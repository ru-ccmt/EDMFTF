...
!
      if(lapw) then
      XAC = PLO(J,JATOM,isi)*DPE(J+1,JATOM,isi) - &
            DPLO(J,JATOM,isi)*PE(J+1,JATOM,isi)
      XAC = XAC*RMT*RMT
      XBC = PLO(J,JATOM,isi)*DP(J+1,JATOM,isi) - &
            DPLO(J,JATOM,isi)*P(J+1,JATOM,isi)
      XBC = -XBC*RMT*RMT
      CLO(J,jlo,JATOM,isi) = XAC*(XAC + 2.0D+0*PI12LO(J,JATOM,isi)) + &
                     XBC*(XBC*PEI(J+1,JATOM,isi) + &
                     2.0D+0*PE12LO(J,JATOM,isi)) + &
                     1.0D+0
...
!.....APW definitions
         if(jlo.eq.1) then
            alonorm=sqrt(1.d0+(P(J+1,JATOM,isi)/PE(J+1,JATOM,isi))**2 &
                 *PEI(J+1,JATOM,isi))
            ALO(J,jlo,JATOM,isi) = 1.d0 /alonorm 
            BLO(J,jlo,JATOM,isi) = -P(J+1,JATOM,isi)/PE(J+1,JATOM,isi) &
                 /alonorm
      ...
      write (6,10) j,alo(j,jlo,jatom,isi),blo(j,jlo,jatom,isi), &
                  clo(j,jlo,jatom,isi)
