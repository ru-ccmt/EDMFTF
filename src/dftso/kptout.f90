subroutine kptout(ss,bname,weight,kkk,kv,jspin,nv,ne)
  ...
  do isi=1,2
     ...
     do j=1,neig 
        ...
        write(ms+10,*)j,en(j)
        !...rearrange
        do n=1,nv(is)-(nlo(1)+nlon(1)+nlov(1))  ! n are reciprocal wave vectors
           vt(n)=vect(n,j,isi)
        end do
	n=n-1
        nn=n
        nl=0
        do ity=1,nat  ! here we deal with local orbitals
           do l=0,lomax
              do jlo=1,ilo(l,ity)
                 do index=1,(2*l+1)*mult(ity)
                    n=n+1
                 ...
                 enddo
              enddo
              if (loorext(l,ity)) then  ! need to add p_{1/2} orbitals as well
                 do index=1,(2*l+1)*mult(ity)
                    n=n+1
                    nl=nl+1
